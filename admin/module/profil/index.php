...
<div class="row">
	<div class="col-sm-3">
		<div class="card card-primary">
			...
			<div class="card-body">
				<img src="assets/img/user/<?= htmlspecialchars($hasil['foto'] ?? '', ENT_QUOTES, 'UTF-8');?>" alt="#" class="img-fluid w-100" />
			</div>
			<div class="card-footer">
                                <form method="POST" action="fungsi/edit/edit.php?foto=user" enctype="multipart/form-data">
                                        <?php echo csrf_field(); ?>
					<input type="file" accept="image/*" name="foto">
					<input type="hidden" value="<?= htmlspecialchars($hasil['foto'] ?? '', ENT_QUOTES, 'UTF-8');?>" name="foto2">
					<input type="hidden" name="id" value="<?= htmlspecialchars($hasil['id_user'] ?? '', ENT_QUOTES, 'UTF-8');?>">
					<br><br>
							...
							<div class="control-group mb-3">
								...
								<div class="input-group">
									<input type="text" class="form-control" style="border-radius:0px;" name="nama"
										data-items="4" value="<?= htmlspecialchars($hasil['nama'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
										required="required" />
								</div>
							</div>
							<div class="control-group mb-3">
							...
							<div class="control-group mb-3">
								...
								<div class="controls">
									<textarea name="alamat" rows="3" class="form-control" style="border-radius:0px;"
										required="required"><?= htmlspecialchars($hasil['alamat'] ?? '', ENT_QUOTES, 'UTF-8'); ?></textarea>
								</div>
							</div>
							<input type="hidden" name="id" value="<?= htmlspecialchars($hasil['id_user'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
							...
							<div class="control-group mb-3">
								...
								<div class="input-group">
									<input type="text" class="form-control" style="border-radius:0px;" name="username"
										data-items="4" value="<?= htmlspecialchars($hasil['username'] ?? '', ENT_QUOTES, 'UTF-8');?>" />
								</div>
							</div>
							<div class="control-group mb-3">
								<label class="control-label" for="typeahead">Password Baru</label>
								<div class="input-group">
									<input type="password" class="form-control" placeholder="Enter Your New Password" id="password" name="pass" data-items="4" value=""
										required="required" />
								</div>
							</div>
							<input type="hidden" class="form-control" style="border-radius:0px;" name="id"
								value="<?= htmlspecialchars($hasil['id_user'] ?? '', ENT_QUOTES, 'UTF-8');?>" required="required" />
