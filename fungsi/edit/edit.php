...
if (!empty($_SESSION['user'])) {
    ...
    if (get_get_param('foto') !== '') {
        ...
        if (move_uploaded_file($tmpName, $uploadDir.$name)) {
            ...
            $data = [$name, $id];
            $sql = 'UPDATE users SET foto=? WHERE users.id_user=?';
            $row = $config->prepare($sql);
            $row->execute($data);
    ...
    if (get_get_param('profil') !== '') {
        ...
        $sql = 'UPDATE users SET nama=?,alamat=?,telepon=?,email=?,NIK=? WHERE id_user=?';
        $row = $config->prepare($sql);
        $row->execute($data);
        echo '<script>window.location="../../index.php?page=user&success=edit-data"</script>';
    }
            ...
            ?>
                ...
                <?php foreach ($hasil1 as $hasil) {?>
                        <tr>
                                ...
                                <td>
                                <a href="fungsi/tambah/tambah.php?jual=jual&id=<?php echo urlencode($hasil['id_barang']);?>&id_kasir=<?php echo urlencode($_SESSION['user']['id_member']);?>&csrf_token=<?php echo urlencode(csrf_get_token());?>"
